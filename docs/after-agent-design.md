...
>
> **Trello 등록 흐름 (Step D-2) — 카드 후보 선택 방식:**
> 1. 업체명 기반으로 `trello.search_cards()` 호출 → 유사 카드 후보 최대 5개 검색 (부분 포함·토큰 매칭)
> 2. `_propose_trello_registration()` — 후보 카드 목록 + 신규 생성 + 건너뜀 버튼 Slack 발송
> 3. 사용자 선택에 따라 분기:
>    - `handle_trello_select_card()` — 기존 카드 선택 → `_register_to_card()` → `trello.add_checklist_items_by_id()` 호출
>    - `handle_trello_new_card()` — 신규 카드 생성 후 등록
>    - `handle_trello_skip()` — 건너뜀
> 4. 체크리스트 항목 포맷: `[담당자] 작업 제목 (기한: YYYY-MM-DD)`
>
> **Trello 카드 조회 (Step D-3):**
> - `handle_trello_search()` — `/트렐로조회` 또는 자연어(`trello_search` 인텐트)로 호출
> - 키워드 있으면 유사 검색, 없으면 전체 목록
