...
> 최종 갱신: 2026-04-02 | 업체명 자동추출 제거(extendedProperties만 사용), 통합 스레드 업데이트 시스템(_meeting_drafts thread_ts 키 기반 영속화), 브리핑 unfurl 비활성화
...
cal.parse_event(ev)  →  meeting dict
         │
         ▼
ev.extendedProperties.private.company 확인
  (일정 생성 시 사용자가 업체를 지정한 경우에만 설정됨)
  ※ 자동 추출 로직 제거됨 (2026-04-02)
         │
    ┌────┴────────────┐
...
_send_briefing()     _send_internal_briefing()
    ...
    │
    ▼  [1단계: 즉시]
build_meeting_header_block() → Slack blocks 즉시 발송 (unfurl_links=False)
_pending_agenda[msg_ts] = (event_id, user_id) 등록
_register_briefing_draft(msg_ts, ...) → _meeting_drafts에도 등록
...
> **2026-04-02 변경**: 일정 제목에서 업체명을 자동 추출하는 로직(`_extract_company_name`, `_extract_company_with_llm`, `_get_internal_products_from_knowledge`)이 제거되었습니다.

현재 외부 미팅 판별은 **Google Calendar `extendedProperties.private.company`** 값의 유무로만 결정됩니다:

```python
company_name = ev.get("extendedProperties", {}).get("private", {}).get("company")
```

업체명이 설정되는 경로:
1. **일정 생성 시**: 사용자가 "업체는 XX이야"라고 명시 → `_create_calendar_event()`에서 `cal.update_event(extended_properties={"private": {"company": ...}})` 호출
2. **스레드 업데이트**: 일정 생성/브리핑 스레드에서 "업체는 XX이야" 답글 → `update_meeting_from_text()` → `cal.update_event()`
...
브리핑 헤더 발송 후 `_pending_agenda[msg_ts]` + `_meeting_drafts[msg_ts]` 이중 등록.
...
### 6.5 통합 스레드 업데이트 (`update_meeting_from_text`)

> **2026-04-02 변경**: 기존 `handle_agenda_reply`(어젠다만 업데이트)가 LLM 기반 `update_meeting_from_text`로 대체되었습니다. 브리핑/일정생성 스레드 모두에서 제목·참석자·장소·어젠다·업체 변경이 가능합니다.

```
스레드 답장 감지 (main.py)
  └─ _meeting_drafts[thread_ts] 존재 확인
       └─ update_meeting_from_text(user_message=text, thread_ts=thread_ts)
            ├─ _generate(merge_meeting_prompt(draft["info"], text)) → LLM 판단
            ├─ is_update=true → 변경 필드별 cal.update_event() 호출
            ├─ "✅ 일정이 업데이트되었습니다." 답장
            └─ draft["info"] 갱신 + _save_meeting_drafts()
```

#### `_meeting_drafts` 통합 드래프트 시스템

- **키**: `thread_ts` (Slack 메시지 ts)
- **영속화**: `data/meeting_drafts.json` (서버 재시작 후에도 유지)
- **TTL**: 7일 (`_cleanup_old_drafts()`, `run_briefing()` 시 실행)
- **등록 시점**:
  - 브리핑 발송 시: `_register_briefing_draft(msg_ts, ...)` → `source: "briefing"`
  - 일정 생성 시: `_create_calendar_event()` → `source: "create"` (reply_ts, thread_ts, user_msg_ts 모두 키로 등록)
- **마이그레이션**: 기존 `_pending_agenda` 항목은 시작 시 자동 마이그레이션
