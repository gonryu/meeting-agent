...
Slack 이벤트·슬래시 커맨드·버튼 액션 핸들러를 모두 등록하는 단일 파일입니다. APScheduler로 6개의 정기 작업을 관리합니다:
...
- `scheduled_meeting_alarm()` — 매분 폴링, 캘린더 미팅 시작 5분 전(±30초) DM 발송 + 자동 세션 바인딩 (`during.bind_event_session()`). `users.meeting_start_alarm_enabled = 0`인 사용자는 건너뜀. `meeting_alarms` 테이블로 (user, event) 단위 중복 발송 방지. Meet 링크가 있으면 메시지에 포함, 없으면 대면 안내(음성·텍스트 업로드 가능)
