...
Slack 이벤트·슬래시 커맨드·버튼 액션 핸들러를 모두 등록하는 단일 파일입니다. APScheduler로 7개의 정기 작업을 관리합니다:

- `scheduled_briefing()` — 매일 09:00 KST, 전체 사용자 브리핑 (`users.briefing_enabled = 0`인 사용자는 건너뜀; 자연어 "브리핑 알람 꺼줘"/"설정" 또는 `/설정` 슬래시 커맨드로 토글. 슬래시 커맨드는 Slack 앱 등록 대기 중 — `TODO.md` 참고)
- `check_transcripts()` — 10분 주기, Drive 트랜스크립트 폴링 (모든 최근 종료 미팅 대상)
...
- `scheduled_meeting_alarm()` — 매분 폴링, 캘린더 미팅 시작 5분 전(±30초) DM 발송 + 자동 세션 바인딩 (`during.bind_event_session()`). `users.meeting_start_alarm_enabled = 0`인 사용자는 건너뜀. `meeting_alarms` 테이블로 (user, event) 단위 중복 발송 방지. Meet 링크가 있으면 메시지에 포함, 없으면 대면 안내. DM에 "🛑 지금 미팅 끝남" 버튼 포함 → 즉시 `end_session()` 진입 (`meeting_end_now` 액션). 활성 세션의 `event_id`와 매칭 안 되면 무시
- `scheduled_fast_transcript_check()` — 2분 주기, 캘린더 종료시각이 지난 활성 세션이 있는 사용자만 대상으로 `_check_transcripts_for_user(min_minutes_ago=0)` 호출. 사용자가 "지금 미팅 끝남" 버튼이나 `/미팅종료`를 안 눌러도 트랜스크립트가 도착하는 즉시(최대 ~2분 지연) 회의록 생성. `bind_event_session()`이 `event_end_iso`를 세션에 저장해 캘린더 추가 호출 없이 비교
...
- `agents/before.py` — `_ontology_enabled(user_id)` 게이팅 후 `_run_briefing_research` ④블록에서 주입. 만료(401)→재등록 DM, 그 외 실패→섹션 생략(브리핑 안 깨짐). 인물 블록(③담당자)은 외부 참석자(사내 도메인 `INTERNAL_DOMAINS` 제외)에 한해 `tools/ontology.person_context`로 "함께한 미팅이력"을 주입한다(`_person_meetings` 게이팅·best-effort).
...
**게이팅:** 토큰 보유는 항상 필수. `ONTOLOGY_BETA_USERS`(쉼표 user_id)가 **비어있으면 GA 모드**(토큰 등록자 누구나 ON — 등록 자체가 옵트인), **값이 있으면 그 allowlist로 제한**(베타/킬스위치). 토큰 없는 사용자는 항상 기존 동작.
...
**흐름:**
- **정기 실행** (매주 금요일 21:00 KST): `scheduled_trello_weekly()` → `send_weekly_report(client)` → `TRELLO_REPORT_CHANNEL`(없으면 `FEEDBACK_CHANNEL`)로 발송
- **수동 트리거**: 호출자에게만 회신 — 채널 @멘션이면 **스레드 답글**, DM이면 **DM**
  - 슬래시 커맨드: `/트렐로주간보고` 또는 `/trello-weekly` (인자로 일수 지정 가능 예: `/트렐로주간보고 14`)
  - 자연어 (`trello_weekly_report` 인텐트): `"주간보고서"`, `"트렐로 주간 보고"`, `"지난 2주 트렐로 보고서"` 등
...
- `agents/trello_report.py` — Trello REST API로 워크스페이스 전 보드 액션·체크리스트 수집, 원본 Markdown/Slack mrkdwn 분리 포맷
  ...
  - `send_weekly_report(slack_client, channel, thread_ts, days, workspace)` — 생성 + Drive 저장 + Slack 발송. `thread_ts` 지정 시 스레드로 회신
  - LLM 배치 호출 2종: Slack용 카드별 한 줄 요약 (`_summarize_comments_one_liner`) + Docs용 긴 코멘트 2줄 요약 (`_summarize_long_comments`, 300자 초과만)
- `tools/drive.create_draft_doc()` — `text/markdown` MIME으로 업로드하여 **Google Docs 서식(H1~H3·굵게·링크·리스트·표) 자동 적용**
