...
7. 섹션은 역할이 다르다 — **같은 사실을 여러 섹션에 반복하지 마라**(가장 적합한 한 곳에만):
   ...
   · deal_context(거래 맥락) = *우리(파라메타)와의* 진행 이력/현황. 1문단 서술.
...
def _initial_prompt(company_name: str, meeting_context: str, ontology_context: str = "") -> str:
    ctx = f"\n\n미팅 맥락:\n{meeting_context}" if meeting_context else ""
    chans = slack_read.biz_channel_list()
    chan_hint = ""
    if chans:
...
def _agent_loop(company_name: str, meeting_context: str, ctx: "ToolContext",
    ...
    tools = _tool_specs()
    messages = [{"role": "user", "content": _initial_prompt(company_name, meeting_context, ontology_context)}]
    called: set = set()
    nudged = False
