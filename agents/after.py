...
# 스레드 등록 흐름 전용 — 작성자·작성일자 등 코멘트 헤더용 메타 (event_id → dict)
_minutes_metadata_cache: dict[str, dict] = {}
...
_THREAD_STRUCTURED_SUMMARY_PROMPT = """다음 슬랙 메시지로 공유된 회의록을 마크다운으로 구조화해 정리하세요.

## 출력 형식 (각 섹션 헤더는 정확히 그대로 사용)
─── 개요 ───
- 회의 대상·일자·참석자 등 기본 정보 (한 줄로)

─── 주요 논의 ───
- 논의 내용을 bullet로 (각 5~25자, 핵심만)

─── 액션 아이템 ───
- [담당자] 작업 내용 (기한이 있으면 괄호로 추가)

─── 후속 논의 포인트 ───
- 다음에 다뤄야 할 미확정 사항이나 후속 미팅 주제

## 규칙
- 각 섹션의 내용이 원문에 명시되지 않으면 **그 섹션 자체를 생략** (헤더도 출력 안 함)
- bullet은 정확히 "- "로 시작
- 추측·창작 금지. 원문에 적힌 사실만 사용
- 액션 아이템 담당자가 불명확하면 [미정]으로 표기
...
def handle_trello_register_from_text(
    ...
    # 4. 회의록 요약·메타 생성을 백그라운드로 시작 — 카드 UI 발송과 병렬 진행.
    # 사용자가 카드 고르는 동안(보통 5초 이상) 캐시가 채워짐. 미완료여도 _register_to_card가
    # 빈 캐시를 그레이스풀하게 처리하므로 안전.
    def _bg_prepare_comment():
        try:
            # 작성자 이름 조회
            author_name = ""
            if parent_author_id:
                try:
                    resp = slack_client.users_info(user=parent_author_id)
                    profile = (resp.get("user") or {}).get("profile") or {}
                    author_name = (profile.get("display_name") or
                                   profile.get("real_name") or "").strip()
                except Exception as e:
                    log.warning(f"작성자 이름 조회 실패: {e}")

            # 작성 일시 (KST) 변환
            date_str = ""
            if parent_msg_ts:
                try:
                    dt = datetime.fromtimestamp(float(parent_msg_ts), tz=KST)
                    date_str = dt.strftime("%Y-%m-%d %H:%M")
                except Exception as e:
                    log.warning(f"작성일 변환 실패: {e}")

            # 구조화 요약 생성 (섹션 마크다운)
            sections = _generate(
                _THREAD_STRUCTURED_SUMMARY_PROMPT.format(minutes=parent_text)
            ).strip()

            # 원문 푸터 (Trello 코멘트 16k 한도 대비 5000자 절단)
            if len(parent_text) > 5000:
                truncated = parent_text[:5000] + "\n... (이하 생략)"
            else:
                truncated = parent_text
            full_body = f"{sections}\n\n─── 원문 ───\n{truncated}"

            _minutes_summary_cache[event_id] = full_body
            _minutes_metadata_cache[event_id] = {
                "author": author_name, "date": date_str,
            }
            log.info(f"스레드 요약 캐시 저장: {event_id} "
                     f"(author={author_name or '?'}, date={date_str or '?'})")
        except Exception as e:
            log.warning(f"회의록 요약 생성 실패 (무시): {e}")
    threading.Thread(target=_bg_prepare_comment, daemon=True).start()
...
def _build_trello_comment(summary_body: str, metadata: dict) -> str:
    """Trello 카드 코멘트 텍스트 조립.

    - 헤더: "📝 회의록 요약"
    - 메타 (작성자·일자) — 둘 다 비어있으면 라인 자체 생략 (자동 등록 흐름 호환)
    - 본문: 구조화된 섹션 + 원문 (스레드 흐름) 또는 평문 요약 (자동 흐름)
    """
    lines = ["📝 회의록 요약"]
    author = (metadata or {}).get("author", "").strip()
    date = (metadata or {}).get("date", "").strip()
    if author or date:
        meta_parts = []
        if author:
            meta_parts.append(f"👤 작성: {author}")
        if date:
...
def _register_to_card(slack_client, *, user_id: str, event_id: str,
    ...
    # 회의록 요약을 카드 코멘트로 추가 — 메타 헤더(작성자·일자) + 본문
    summary = _minutes_summary_cache.pop(event_id, "")
    metadata = _minutes_metadata_cache.pop(event_id, {})
    if summary:
        try:
            comment = _build_trello_comment(summary, metadata)
            trello.add_comment_by_id(user_id, card_id, comment)
        except Exception as e:
