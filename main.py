...
- register_ontology: 온톨로지(lib-mesh) 사내 지식 연결/등록 (예: "온톨로지 등록", "온톨로지 연결", "온톨로지 토큰 등록", "lib-mesh 연결", "온톨로지 설정", "온톨로지 등록해줘", "온톨로지 붙여줘")
...
def _route_message(text: str, client, user_id: str, channel: str = None,
    ...
    if intent == "briefing":
    ...
    elif intent == "register_ontology":
        # 온톨로지 연결은 개인 토큰 등록이므로 항상 DM으로 랜딩 링크 발송
        ...
        if not parent_text:
            ...
            return
        threading.Thread(
            target=after.handle_trello_register_from_text,
            args=(client,),
