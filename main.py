...
- search_minutes: 업체·기간 기반 회의록 검색 (예: "카카오 회의록 찾아줘", "지난달 회의록", "삼성전자 3월 회의록", "KISA 회의록 검색")
...
- search_minutes: {{"query": "검색 키워드 (업체명, 기간 등 원본 그대로)"}}
...
def _search_minutes(client, *, user_id: str, query: str,
    ...
    """업체·기간 기반 회의록 검색"""
    # 기간 파싱: YYYY-MM 또는 YYYY-MM-DD 패턴 추출
    ...
    if range_match:
        ...
        company = query[:range_match.start()].strip() + " " + query[range_match.end():].strip()
    ...
    # YYYY-MM 월 단위
        ...
        company = query[:month_match.start()].strip() + " " + query[month_match.end():].strip()
        company = company.strip()
    ...
    # "지난달", "이번달" 등 자연어 기간
        ...
        company = query.replace("지난달", "").replace("저번달", "").strip()

    # 업체명만 남기기: 불필요한 토큰 제거
    for token in ["회의록", "검색", "찾아줘", "보여줘", "조회"]:
        company = company.replace(token, "").strip()

    results = user_store.search_meetings(
        user_id=user_id,
    ...
    for r in results:
        ...
        if link:
            lines.append(f"• *{date}* <{link}|{title}>{company_label}")
        else:
            lines.append(f"• *{date}* {title}{company_label}")
